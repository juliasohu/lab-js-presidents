...
function getNames(presidentsArr) {
  const presidentNames = presidentsArr.map((element) => {return element.name})
...
function getDemocraticPresidents(presidentsArr) {
  const democraticPresidents = presidentsArr.filter((element, index, arr) => {
...
function  countYearsInOffice(presidentsArr) {
  const yearsinOffice = presidentsArr.reduce((acc, currentValue) => {
   if (currentValue.leftOffice !== null) {
    let yearDiff = currentValue.leftOffice - currentValue.tookOffice;
    return acc + yearDiff
   } else {return acc}
  }, 0)
...
function sortPresidentsByBirthYear(presidentsArr) {
  let sortedPresidentsByYear = presidentsArr.sort(function(a, b){
    return a.birthYear - b.birthYear
  })
  return sortedPresidentsByYear
}
